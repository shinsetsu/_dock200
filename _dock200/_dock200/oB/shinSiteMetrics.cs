  ...
  public class shinSiteMetrics {
    ...
    internal void PageEventsIncrement(_DBC _dbContext) {
      try {
        ...
        if (siteMetrics == null) { _dbContext.Add(new shinSiteMetrics() { }); }

        if (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development") {
          siteMetrics.pageViewsDebug++; _dbContext.Update(siteMetrics);
        } else { siteMetrics.pageViewsRelease++; _dbContext.Update(siteMetrics); }
